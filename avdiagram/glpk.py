...
class Constraint(object):
    def __init__(
        self, int_id: int, l: List[Term], eq_or_le_or_ge: str, bound: float
    ) -> None:
        self.l = l
...
class Problem(object):
    ...
    def addConstraint(
        self, l: List[Tuple[Var, float]], eq_or_le_or_ge: str, bound: float
    ) -> None:
    ...
    def renderToGLPK(self, out: TextIO) -> None:
        ...
        out.write(
            "p mip {3} {0} {1} {2}\n".format(
                len(self.cons), len(self.vars), self.countEntries(), mm
            )
        )
        for c in self.cons:
            i = c.int_id
        ...
        for var in self.vars:
            ...
            if var.lbound is None and var.ubound is None:
                ...
                not var.lbound is None
                and not var.ubound is None
                and var.lbound < var.ubound
            ):
                out.write(

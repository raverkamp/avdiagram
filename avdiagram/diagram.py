...
class DVar(object):
    def __init__(self, parent: "Diagram", name:str, lbound:Optional[float], ubound:Optional[float]):
        self.id = newid()
        self.name = name
        self.parent = parent
        self.lbound = lbound
...
class Constraint(object):
    ...
    def __init__(self, name:str, sums: List[Tuple[float,DVar]], op: Relation, const: float):
        self.name = name
        self.op = op
        self.sums = sums
        self.const = const
...
class Point (Base):
    ...
    def to_svg(self, x:float, y:float, w:float, h:float)->Tag:
        return translate(x,y,[line(-10,0,10,0,color="red"),
                              line(0,-10,0,10,color="red"),
                              text(10,-10,self._name)])

#  diagram table
class DTable(Thing) :
    def __init__(self, name: str,  table:Table, font: Font, x:DVar, y:DVar, w:DVar, h:DVar)->None:
    ...
    def to_svg(self,x:float,y:float, w:float, h:float)->Tag:
        ...
        li = line(0,bl+ self.font.font_size * 0.2, w, bl + self.font.font_size * 0.2,width=2)
...
class DText(Thing) :
    def __init__(self,name:str,txt:str,font: Font, x:DVar, y:DVar, w:DVar, h:DVar)->None:
        super().__init__(name,x,y,w,h)
        self.font = font
        self.txt = txt
...
class DTextLines(Thing) :
    def __init__(self,
                 ...
                 font: Font,
                 border: float,
                 x:DVar, y:DVar, w:DVar, h:DVar)->None:
        super().__init__(name,x,y,w,h)
        self.tb =  border
        self.font = font
        self.text_lines = txt
...
def load_tables(fname:str,tab_prefix:Optional[str]=None,col_prefix:Optional[str]=None)-> dict[str,Table] :
    with open(fname,"r",newline='') as f:
        ...
        lines = list(reader)
        a = filter(lambda x : len(x) > 1 and (not tab_prefix or x[1].startswith(tab_prefix)),lines)
        a2 = map(lambda x : (x[0],strip_tab_prefix(x[1]),stripcolprefix(x[2]),x[3]),a)
...
def cpfor(x1:float,y1:float,mx:float,my:float,x2:float,y2:float,d:float)-> Tuple[float, float] :
    dx = x2-x1
    dy = y1-y2
    da = vlen(dx,dy)
...
class Diagram(object) :
    ...
    def add_constraint(self, name:str, sums: List[Tuple[float,DVar]], op: Relation, const: float)->None:
    ...
    def pos(self,name:Union[str,Base],x:Optional[float]=None,y:Optional[float]=None)->None:
        obj = self.k2o(name)
        if not x is None:
    ...
    def point(self,name:str,x:Optional[float]=None,y: Optional[float]=None)->Point :
    ...
    def cline(self,
              ...
              t2: Union[str, Thing],
              p2: float,
              liste:List[Union[str,Point]]=[])->None :
        a1 = self.k2o(t1)
    ...
    def posfornum(self,values:dict[str, float], obj:Thing,p:float)->Tuple[float, float]:
    ...
    def bseg(self,x1:float,y1:float,mx:float,my:float,x2:float,y2:float,d:float)->str :
        (cx,cy) = cpfor(x1,y1,mx,my,x2,y2,d)
        return "{0} {1} {2} {3}  ".format(cx,cy,mx,my)

    def bpath(self, values:dict[str, float], obj1:Thing,p1:float,obj2:Thing,p2:float,liste:List[Point])->Tag :
        d = 100
    ...
    def enforce(self)-> Tuple[glpk.Problem, dict[str,DVar] ,  dict[str,glpk.Var]]:
        ...
        for cons in self.constraints:
            ...
            operator = "GE" if cons.op == Relation.GE else ("LE" if cons.op == Relation.LE else "EQ")

...
def line(x1:float,y1: float,x2: float,y2: float,color:str="#000",width:float=1 ,marker_end: Optional[str] = None)->Tag :
...
def path(pa:str,color:str="#000",width:float=1,marker_end:Optional[str]= None) -> Tag:
...
def render_file(name:str,w:float,h:float,things:Stuff)->None :
    with open(name,"w",encoding="UTF-8") as f:
        f.write(SVG_HEADER+"\n")
        svg = Tag("svg",{'xmlns':"http://www.w3.org/2000/svg",
                      'xmlns:xlink':"http://www.w3.org/1999/xlink",
                      "width":str(w),
                         "height":str(h)}, [defs,things])
        l:List[str] = []
        render(l,svg)
        f.write("".join(l))
